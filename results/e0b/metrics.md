...
- **Rg_raw** = **71.4 rps** at C=16 (best goodput with P95 ≤ 1.5 × C=1 P95).
- C=32 is not the operating point (P95 718 ms).
- **Experimental Rg** = **0.4 rps** (gateway token-bucket). This is an org safety budget so E1–E4 can saturate G_strong without first hitting Maverick (~1.8 rps) or raw ApplyGuardrail (~71 rps).

ApplyGuardrail P50 (~215 ms) is much faster than Nova Micro G_light P50 (~544 ms). Raw G_strong is not the bottleneck; the scheduled resource is the gateway quota.
